...
import com.example.demo.security.services.AccountServiceImp;
...
public class SecurityConfig{
   private final PasswordEncoder passwordEncoder;
    ...
    public UserDetailsService userDetailsService() {
        return username -> {
            com.example.demo.security.entities.User userEntity = accountServiceImp.loadUserByUsername(username);
            if (userEntity == null) {
                throw new UsernameNotFoundException("User not found with username: " + username);
            }
            Role roleName = accountServiceImp.listRole(userEntity);

            return org.springframework.security.core.userdetails.User.builder()
                    .username(userEntity.getUsername())
                    .password(userEntity.getPassword())
                    .roles(String.valueOf(roleName))
                    .build();
        };
    }
}
